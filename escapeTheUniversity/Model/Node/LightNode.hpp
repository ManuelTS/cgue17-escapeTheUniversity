...
public:
	...
	struct Light{ // Due to openGL byte ordering only vec4 usage
		...
		glm::vec4 diffuse = glm::vec4(0.0f, 0.0f, 0.0f, 0.01f); // rgb = rgb of diffuse light, a = ambient light coefficient to decrase the diffuse light and use it as ambient one
