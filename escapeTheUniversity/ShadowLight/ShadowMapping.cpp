...
void ShadowMapping::renderDepthMap(Node* root, LightNode* ln, const float farPlane, const float FOV, const unsigned int screenWidth, const unsigned int screenHeight)
{
	...
	glm::mat4 lightProjection = glm::perspective(frustum->degreesToRadians(FOV), (GLfloat)SHADOW_WIDTH / (GLfloat)SHADOW_HEIGHT, near_plane, farPlane); // note that if you use a perspective projection matrix you'll have to change the light position as the current light position isn't enough to reflect the whole scene
