...
void ShadowMapping::renderInDepthMap(Node* root, LightNode* ln, const float FOV, const unsigned int screenWidth, const unsigned int screenHeight)
{
	...
	glm::mat4 lightProjection = glm::perspective(frustum->degreesToRadians(120.0f), (float)SHADOW_WIDTH / (float)SHADOW_HEIGHT, near_plane, ln->light.specular.w * 2); // note that if you use a perspective projection matrix you'll have to change the light position as the current light position isn't enough to reflect the whole scene
	...
	glm::vec3 lightFront = glm::normalize(glm::vec3(0.017f, -1.0f, 0.0f));
	glm::vec3 lightUp = glm::normalize(glm::vec3(1.0f, 0.017f, 0.021f)); // look down
